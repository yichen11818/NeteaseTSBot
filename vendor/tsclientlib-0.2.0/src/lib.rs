...
impl Connection {
	...
	async fn connect_to(
		...
		packet.write_arg("client_nickname", &options.name.as_ref());
		packet.write_arg("client_version", &client_version);
		packet.write_arg("client_platform", &client_platform);
		packet.write_arg(
			"client_input_hardware",
			if options.input_hardware_enabled { &"1" } else { &"0" },
		);
		packet.write_arg(
			"client_output_hardware",
			if options.output_hardware_enabled { &"1" } else { &"0" },
		);
		packet.write_arg("client_default_channel", &options.channel.as_deref().unwrap_or_default());
		packet.write_arg("client_default_channel_password", &default_channel_password);
		packet.write_arg("client_server_password", &password);
		packet.write_arg("client_meta_data", &"");
		packet.write_arg("client_version_sign", &client_version_sign);
		packet.write_arg("client_key_offset", &counter);
		packet.write_arg("client_nickname_phonetic", &"");
		packet.write_arg("client_default_token", &"");
		packet.write_arg("hwid", &options.hardware_id.as_ref());
