...
impl InnerLicense {
	pub fn type_id(&self) -> u8 {
		match *self {
			...
			InnerLicense::Code { .. } => 3,
			InnerLicense::Opaque { type_id, .. } => type_id,
...
impl License {
	...
	pub fn parse(data: &[u8]) -> Result<(Self, usize)> {
		...
		let (inner, extra_len) = match data[33] {
			...
			8 => {
				...
				let _license_type =
					LicenseType::from_u8(data[42]).ok_or(Error::UnknownLicenseType(data[42]))?;
				let property_count = usize::from(data[43]);
				let mut pos = 44;
				for _ in 0..property_count {
					if pos >= data.len() {
						return Err(Error::TooShort);
					}
					let property_len = usize::from(data[pos]);
					pos += 1;
					if data.len() < pos + property_len {
						return Err(Error::TooShort);
					}
					pos += property_len;
				}
				let opaque = data[42..pos].to_vec();
				(InnerLicense::Opaque { type_id: 8, data: opaque }, pos - MIN_LEN)
			}
	...
	pub fn write(&self, mut w: &mut dyn Write) -> Result<()> {
		...
		match self.inner {
			...
			InnerLicense::Opaque { ref data, .. } => {
				w.write_all(data).map_err(Error::Serialize)?;
			}
...
mod tests {
	...
	#[test]
	fn parse_ts6_license_chain() {
		let licenses = Licenses::parse_ignore_expired(&base64::decode("").unwrap()).unwrap();
		assert_eq!(licenses.blocks.len(), 3);
		assert!(matches!(licenses.blocks[0].inner, InnerLicense::Intermediate { .. }));
		assert!(matches!(licenses.blocks[1].inner, InnerLicense::Opaque { type_id: 8, .. }));
		assert!(matches!(licenses.blocks[2].inner, InnerLicense::Ephemeral));
	}
